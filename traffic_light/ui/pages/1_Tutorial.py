...
def tutorial_pages():
    ...
    st.write(f"""This project aims to tackle a problem commonly found in optimization functions, such as those used in Machine Learning, Artificial Intellegence, and Evolutionary Computing.
             ...
             \nSpecification Gaming, for the purpose of this project, is defined as: A situation where an optimization system, such as a Neural Network or Evolutionary Algorithm, produces a result which performs well according to some objective (i.e,\"maximize fitness\"\solution quality) by doing something unexpected or undesired.
    ...
    col_3.image("traffic_light/ui/resources/SG_example_images/slide_to_the_right.gif", caption=f"An evolved creature \'discovered\' and exploit in the physics engine that allowed it to simply glide to the side instead of running.")
    ...
    st.write(f"""From the above examples a common theme emerges. All of the products of optimization found unexpected, and undesirable, methods of technically satisfying their optimization function while not having the desired emergent properties or behaviors.
             \nWhile some of these exmaples may be ammsing, unfortunately not all examples are so funny. In more serious sitations agents trained to drive cars or control planes may adopt niche behavior that scores well according to some optimization process but would put people at dire risk.
             \nTo go further, when these issues occur typically they are discovered at the end of long and expensive optimization processes such as evolutionary runs or neural network trainings. Therefore, specification gaming can also represent a significant and expensive time loss when it occurs. 
