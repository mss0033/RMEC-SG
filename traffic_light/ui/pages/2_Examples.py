...
def example_pages():
    ...
    st.write(f"""Below will be a series of clip sets that show the flow of simulated traffic in a simple simulated city. The logic which controls the traffic lights within the city has been computationally optimized to enhance the flow of traffic in the simple model city. The metric used to compare one set of traffic light logic to another is the time it takes for a simulation to complete.
             ...
             \nAll the lights at all the intersections in the city are grouped and considered a set. The set is scored based on how long it takes a sequence of vehicles to travel through the city when compared to a baseline. Therefore, a lower score means a shorter total time compared to the baseline, which is better (lower score = shorter time => better than baseline, higher score = longer time => worse than baseline).
             \nEach set is run through two traffic scenarios. One scenario involves vehicles traveling along a hand-crafted route, where vehicles travel in a \'stairstep\' pattern from the bottom left to the upper right corner and then return to the bottom left of the simulated city. The other scenario is a pattern of randomly generated traffic in the same city.
             \nTo make comparison easier, a baseline set has been created. The performance of the baseline set is what should be considered 'normal' performance. Sets that perform better than the baseline will be subject to your scrutiny, while sets that perform worse will be spared your gaze, for now.
             \nWhat you will see in each pair of clips is a view of traffic simulation software; it models a very basic city laid out as a fully connected grid, where the outer edge roads form a closed loop. In the simulation software, cars are represented as yellow triangles that travel along a pre-determined route through the city. The vehicles are spawned into the city according to the set traffic pattern, and leave the city when they complete their route. Therefore, when traffic is flowing smoothly the vehicles (yellow triangles) will move through the city without encountering gridlock.
    ...
    col_1.caption(f"Let\'s call the above set Orin. As you can see, Orin successfully flows the stairstep traffic without causing any gridlock. Orin represents the baseline time performance. Please note, the clip starts at a normal speed and then speeds up so the full performance of the set can be seen.")
    # col_1.markdown(
    ...
    col_2.caption(f"In the random traffic scenario, Orin also successfully flows the random traffic without causing any gridlock. Please note, the clip starts at a normal speed and then speeds up so the full performance of the set can be seen.")
    # col_2.markdown(
    #     f'<figure><img src="data:image/gif;base64,{gif_from_local_file(filepath=f"traffic_light/ui/resources/individual_sim_videos/grid_network_original_random.gif")}" width="100%" height="100%"><figcaption>In the random traffic scenario, Orin also successfully flows the random traffic without causing any gridlock. Please note, the clip starts at a normal speed, and then speads up so the full performance of the set can be seen.</figcaption></figure>',
    #     unsafe_allow_html=True,
    # )
    st.markdown("---")
    col_3, col_4 = st.columns(2)
    col_3.video(f"traffic_light/ui/resources/individual_sim_videos/gen_10_grid_network_{2}_stairstep_video.mp4")
    col_3.caption(f"Let\'s call the above set Charlie. As you can see, Charlie successfully flows the stairstep traffic without causing any gridlock. Charlie even flows this traffic slightly faster than Orin. This is a good sign as that is what we want. Please note, the clip starts at a normal speed and then speeds up so the full performance of the set can be seen.")
    # col_3.markdown(
    ...
    #     unsafe_allow_html=True,
    # )
    col_4.video(f"traffic_light/ui/resources/individual_sim_videos/gen_10_grid_network_{2}_random_video.mp4")
    col_4.caption(f"In the random scenario, Charlie also successfully flows the random traffic without any gridlock, and only slightly slower than the baseline. This kind of behavior is acceptable, as trade-offs in performance are expected. Please note, the clip starts at a normal speed and then speeds up so the full performance of the set can be seen.")
    # col_4.markdown(
    #     f'<figure><img src="data:image/gif;base64,{gif_from_local_file(filepath=f"traffic_light/ui/resources/individual_sim_videos/gen_10_grid_network_{2}_random.gif")}" width="100%" height="100%"><figcaption>In the random scenario, Charlie also successfully flows the random traffic without any gridlock, and only slightly slower than the baseline. This kind of behavior is acceptable, as trade offs in performance are expected. Please note, the clip starts at a normal speed, and then speads up so the full performance of the set can be seen.</figcaption></figure>',
    #     unsafe_allow_html=True,
    # )
    # st.markdown("---")
    # col_5, col_6 = st.columns(2)
    # col_5.video(f"traffic_light/ui/resources/individual_sim_videos/gen_10_grid_network_{0}_stairstep_video.mp4")
    # col_5.caption(f"Finally, let\'s call the above set Riley. As you can see, Riley successfully flows the stairstep traffic without causing any gridlock. Additionally, Riley flows all the traffic much faster than the baseline, great! Please note, the clip starts at a normal speed, and then speads up so the full performance of the set can be seen.")
    # # col_5.markdown(
    ...
    # #     unsafe_allow_html=True,
    # # )
    # col_6.video(f"traffic_light/ui/resources/individual_sim_videos/gen_10_grid_network_{0}_random_video.mp4")
    # col_6.caption(f"However, Riley didn\'t get that performance for free. In the clip above, you can see that Riley is not able to flow the random traffic without causing gridlock. While some perfomance trade offs are acceptable, we don\'t want a set that ruins the perfomance of one scenario to gain an advantage in another. Please note, the clip starts at a normal speed, and then speads up so the full performance of the set can be seen.")
    # # col_6.markdown(
