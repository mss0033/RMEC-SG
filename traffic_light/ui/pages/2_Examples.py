...
def example_pages():
    ...
    st.write(f"""Below will be a series of clip sets which show the flow of simulated traffic in a simple simulated city. The logic which controlls the traffic lights within the city has been computationally optimized to enhance the flow of traffic in the simple model city. The metric used to compare one set of traffic light logic to another is the time it takes for a simulation to complete.
             ...
             \nTo make comparison easier, a baseline set has been created. The performance of the baseline set is what should be considered 'normal' performance. Sets which perform better than the baseline will be subject to your scrutiny, while sets that perform worse will be spared your gaze, for now.
             \nWhat you will see in each pair of clips is a view of traffic simulation software; in it modeled a very basic city laid out as a fully connected grid, where the outer edge roads form a closed loop. In the simulation software, cars are represented as yellow triangles which travel along a pre-determined route through the city. The vehicles are spawned into the city according to the set traffic pattern, and leave the city when they complete their route. Therefore, when traffic is flowing smoothly the vehicles (yellow triangles) will move through the city without encountering gridlock.
             \nBelow are clips of the simulated traffic situations described above. In each pair of clips, a single set is controlling the logic of the city's traffic lights.
    ...
    col_1.caption(f"Let\'s call the above set Orin. As you can see, Orin successfully flows the stairstep traffic without causing any gridlock. Orin represents the baseline time perfomance. Please note, the clip starts at a normal speed, and then speads up so the full performance of the set can be seen.")
    # col_1.markdown(
    ...
    col_2.caption(f"In the random traffic scenario, Orin also successfully flows the random traffic without causing any gridlock. Please note, the clip starts at a normal speed, and then speads up so the full performance of the set can be seen.")
    # col_2.markdown(
